...
class TestStoneGroupHandler(unittest.TestCase):
    ...
    def test_groups_creation(self):
        ...
        black_stones = [Stone(pos, Color.BLACK) for pos in black_pos]
        white_stones = [Stone(pos, Color.WHITE) for pos in white_pos]
    ...
    def test_group_fusion(self):
        board = Board()
        board.place_stone(Stone((0, 0), Color.BLACK))
        board.place_stone(Stone((0, 2), Color.BLACK))
        board.place_stone(Stone((1, 1), Color.BLACK))
