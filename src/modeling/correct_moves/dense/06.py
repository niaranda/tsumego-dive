...
# try optimizers, reduce batch size
...
def build_model(optimizer):
    ...
    model.compile(optimizer=eval(optimizer + "(learning_rate=0.00001)"),
...
for i in range(8):
    ...
    model.fit(X_train, y_train, epochs=100, validation_split=0.3, batch_size=32,
              callbacks=[TensorBoard(f"./tb_logs/correct_dense/06/{optimizer}"), ])

    model.save_weights(f"models/correct_dense06_{optimizer}")
